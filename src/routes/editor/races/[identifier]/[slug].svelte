...
<script>
    ...
    async function on_changed_title(event) {
        ...
        const race = await races.query_race($query.query.identifier).exec();
        await query.atomicSet("title", event.detail.value);

        const slug = query.get_title_slug();
        sapper.goto(`/editor/races/${$page.params.identifier}/${slug}`, {replaceState: true});*/
    }
</script>

<SpinnerOverlay active={$query.preloading}>
    {#if $query.race}
        <div
